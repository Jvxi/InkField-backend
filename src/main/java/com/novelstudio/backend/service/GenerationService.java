...
import com.novelstudio.backend.exception.GenerationCancelledException;
import com.fasterxml.jackson.databind.JsonNode;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.novelstudio.backend.model.AiSettings;
import com.novelstudio.backend.model.Chapter;
import com.novelstudio.backend.model.CharacterProfile;
import com.novelstudio.backend.model.ForeshadowingItem;
import com.novelstudio.backend.model.OnboardingState;
import com.novelstudio.backend.model.ProjectMeta;
import com.novelstudio.backend.model.ChapterGenerationResponse;
import com.novelstudio.backend.model.ComplianceReport;
import com.novelstudio.backend.model.NovelTypeInfo;
import com.novelstudio.backend.model.OutlineNode;
import com.novelstudio.backend.model.Project;
import com.novelstudio.backend.model.PublishPlatformInfo;
import com.novelstudio.backend.service.PromptBuilder.GenerationContext;
...
public class GenerationService {
    ...
    public ChapterGenerationResponse generateChapter(String chapterId) {
        try {
            GenerationSession session = prepareSession(chapterId);
        ...
        try {
            ...
            try {
                sendStreamEvent(emitter, "error", Map.of(
                    "message",
                    exception.getMessage() == null ? "Stream generation failed." : exception.getMessage()
                ));
    ...
    public void streamReviewChapters(SseEmitter emitter) {
        ...
        try {
            Project project = projectStore.loadActiveProject().project();
            ...
            for (int i = 0; i < total; i++) {
                ...
                String prompt = "你是一名专业的网文编辑。请审查以下小说章节正文，找出其中不属于小说正文的内容。\n\n"
                    + "不属于正文的内容包括但不限于：\n"
                    + "- 作者备注、创作说明、写作提示（如\"此处需要展开\"、\"待修改\"等）\n"
                    + "- AI生成残留的元数据或指令（如\"以下是本章内容\"、\"希望你喜欢\"等）\n"
                    + "- 大纲描述混入正文（如\"本章要点：...\"、\"章节目标：...\"等）\n"
                    + "- 格式说明或排版指令\n"
                    + "- 与故事无关的评论、分析、总结\n"
                    + "- 重复的段落或句子\n"
                    + "- 明显的逻辑错误或前后矛盾的内容\n\n"
                    + "请严格按以下JSON数组格式输出问题列表。如果没有问题，输出空数组 []。\n"
                    + "[{\"original\":\"有问题的原文片段\",\"description\":\"问题说明\",\"suggestion\":\"建议修改为\"}]\n\n"
                    + "注意：\n"
                    + "- original 必须是原文中精确存在的文字\n"
                    + "- 如果原文完全正常，输出 []\n"
                    + "- 不要修改正常的文学表达，只清除不属于正文的杂质内容\n\n"
                    + "章节正文：\n" + draft;

                try {
                    StringBuilder resultBuilder = new StringBuilder();
                    streamRemoteModel(project, prompt, null, resultBuilder::append, cancelled);
                    String raw = resultBuilder.toString().trim();

                    List<Map<String, String>> issues = parseReviewIssues(raw);
            ...
            try {
                sendStreamEvent(emitter, "error", Map.of(
                    "message", e.getMessage() == null ? "审查失败" : e.getMessage()
                ));
            } catch (IOException ignored) {
            }
    ...
    public Map<String, Object> testConnection(AiSettings settings) {
        ...
        try {
            ...
            return Map.of("ok", false, "message", exception.getMessage() == null ? "连接失败" : exception.getMessage());
        }
    }
    ...
    private Project autoGenerateMetadata(Project project, String chapterId, String draft, List<String> warnings) {
        ...
        try {
            ...
            warnings.add("自动生成标题/摘要/目的失败：" + e.getMessage());
            return project;
        }
    }
    ...
    private String generateDraftContent(
        ...
        if (isRemoteModelReady(session.project.aiSettings())) {
            try {
                ...
                warnings.add("远程生成失败，已回退本地模板：" + exception.getMessage());
            }
        }
    ...
    private ChapterGenerationResponse finalizeGeneration(
        ...
        if (project.meta().strictMode() && !compliance.passed() && provider.startsWith("openai-compatible")) {
            try {
                ...
                warnings.add("严格模式重试失败：" + exception.getMessage());
            }
        }
